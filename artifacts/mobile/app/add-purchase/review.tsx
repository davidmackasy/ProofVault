...
import { hasUsefulExtractedDetails, isNotReceiptImage } from "@/lib/receiptPipeline";
...
const FIELD_PLACEHOLDERS: Partial<Record<keyof typeof EMPTY_FORM, string>> = {
  purchaseDate: "Add purchase date",
  purchaseTime: "Add purchase time",
  totalAmount: "Add total amount",
  returnDeadline: "Add return deadline",
  warrantyExpiry: "Add warranty expiry",
};
...
export default function ReviewDetailsScreen() {
  ...
  useEffect(() => {
    ...
    setForm((f) => ({
      ...f,
      itemName:
        extractionResult.suggestedItemName ??
        extractionResult.itemName ??
        f.itemName,
      ...
      returnPolicyText:
        extractionResult.returnPolicyText ??
        extractionResult.suggestedReturnPolicyText ??
        f.returnPolicyText,
      warrantyText:
        extractionResult.warrantyText ??
        extractionResult.suggestedWarrantyText ??
        f.warrantyText,
    }));
  }, [extractionResult]);

  const notReceipt =
    readStatus === "not_receipt" || isNotReceiptImage(extractionResult);

  const notice = useMemo(() => {
    if (notReceipt) {
      return {
        tone: "warning" as const,
        text: "We couldn't find a receipt in this image.",
      };
    }
    if (hasUsefulExtractedDetails(extractionResult)) {
      return {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        {notReceipt ? (
          <View style={styles.notReceiptActions}>
            <TouchableOpacity
              style={[styles.actionBtn, { backgroundColor: colors.primary }]}
              onPress={() => router.replace("/add-purchase/camera" as any)}
              activeOpacity={0.85}
            >
            ...
            <TouchableOpacity
              ...
              onPress={() => router.replace("/add-purchase/manual" as any)}
              activeOpacity={0.85}
            >
